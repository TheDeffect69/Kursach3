...
ChatWindow::ChatWindow(DatabaseLogger* logger, QWidget *parent)
    : QWidget(parent), dbLogger(logger), tcpServer(new QTcpServer(this)), tcpSocket(nullptr) {
...
void ChatWindow::setupUI() {
    ...
    serverLayout->addWidget(new QLabel("Listen Port:"));
    ...
    clientLayout->addWidget(new QLabel("Peer Host:"));
    peerHostInput = new QLineEdit("127.0.0.1");
    clientLayout->addWidget(peerHostInput);
    clientLayout->addWidget(new QLabel("Peer Port:"));
...
void ChatWindow::startServer() {
    ...
    if (tcpServer->listen(QHostAddress::Any, port)) {
        appendMessage("System", "Started listening on port " + QString::number(port));
        listenButton->setEnabled(false);
        listenPortInput->setEnabled(false);
    } else {
        QMessageBox::critical(this, "Error", "Could not start server: " + tcpServer->errorString());
    }
}
...
void ChatWindow::connectToPeer() {
    ...
    appendMessage("System", "Connecting to " + host + ":" + QString::number(port) + "...");
}
...
void ChatWindow::newConnection() {
    ...
    appendMessage("System", "Peer connected from " + tcpSocket->peerAddress().toString());
}
...
void ChatWindow::socketError(QAbstractSocket::SocketError error) {
    Q_UNUSED(error);
    if (tcpSocket) {
        appendMessage("System", "Socket error: " + tcpSocket->errorString());
    }
}
